...
class AttackTest : FunSpec() {
    ...
    private suspend fun aDeadSoldier(alliedWith: Agent? = null): Soldier {
    ...
    private suspend fun Agent.attacking(victim: Soldier, with: Soldier): OutputSpy {
        val outputSpy = OutputSpy()
        val useCase: Attack = AttackUseCase(context)
        useCase.attack(
            Attack.Request(agent = this, attackerId = with.id, victimId = victim.id),
            output = outputSpy
        )
        return outputSpy
    }
    ...
    private suspend infix fun OutputSpy.shouldLeaveTheVictimAt(expectedHealth: SoldierHealth) {
        ...
        val savedVictim = context.getBattle().soldiers.find { it.id == result.victim.id }
        savedVictim.shouldNotBeNull()
        withClue("victim in battle should have expected health") {
            savedVictim.health shouldBe expectedHealth
        }
        withClue("victim in result should have expected health") {
            result.victim.health shouldBe expectedHealth
        }
    }

    private suspend infix fun OutputSpy.shouldLeaveTheAttackerAt(expectedHealth: SoldierHealth) {
        val result = result ?: fail("No result received.  $validationErrors")
        val savedAttacker = context.getBattle().soldiers.find { it.id == result.attacker.id }
        savedAttacker.shouldNotBeNull()
        withClue("attacker in battle should have expected health") {
            savedAttacker.health shouldBe expectedHealth
        }
        withClue("attacker in result should have expected health") {
            result.attacker.health shouldBe expectedHealth
        }
    }

    class OutputSpy : Attack.Output {
